...
The application features full local browser persistence via IndexedDB or centralized SQLite storage using an optional backend server. It supports rich features such as prompt templates, dynamic CSS themes, markdown previews, text-to-speech synthesis (TTS), token counts, interactive log-probability overlays, a keyboard-driven Quick Switcher for fast session switching, and a dedicated modal for session management with search and metadata-based sorting.
