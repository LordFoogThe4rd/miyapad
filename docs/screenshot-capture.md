...
- `src/hooks/useScreenshotCapture.ts` — Core logic: reads selected text, uses `promptChunks` from context to color-code AI vs User text, builds a hidden HTML layout, renders to PNG via `html-to-image`, opens result in a new tab.
- `src/components/modals/ScreenshotModal.tsx` — Settings modal (12 fields: session name/date toggles, background URL/color, fonts, colors, avatar URL).
