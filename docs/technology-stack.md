...
- **Type System**: TypeScript 6 with `strict: true` enabled. Uses `noEmit` mode — Parcel handles transpilation during bundling; standalone `tsc --noEmit` is used only for type checking.
